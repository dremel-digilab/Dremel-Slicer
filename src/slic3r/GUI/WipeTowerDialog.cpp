...
{
    std::string icon_path = (boost::format("%1%/images/Dremel3DSlicerTitle.ico") % Slic3r::resources_dir()).str();
...
,m_matrix(matrix), m_min_flush_volume(extra_flush_volume), m_max_flush_volume(Slic3r::g_max_flush_volume)
{
    ...
    {
        wxString message = _L("Dremel 3D Slicer would re-calculate your flushing volumes every time the filaments color changed. You could disable the auto-calculate in Dremel 3D Slicer > Preferences");
